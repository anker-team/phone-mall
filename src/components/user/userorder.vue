    ...
    <div class="page">
        ...
        <div class="main">
            <div class="userOrder-tooltop">
                ...
                <span class="item" v-on:click="getCategory(index)" v-else>{{item.value}}</span>
            </div>
            <div class="userOrder-content">
                <div class="order-block" v-for="order in current_list.data">
                    <div class="otop">
                        <span>订单号：319280174013</span>
                        <span class="fr fs12 state">{{order.state}}</span>
                    </div>
                    <div class="ocontent">
                        <div class="order-item" v-for="(product,idx) in order.goods">
                            ...
                            <div class="info">
                                ...
                                <div class="price c-fc1">
                                    <sub>￥</sub>{{product.goodsPrice}}
                                </div>
                                <div class="spec">
                                    {{product.spec}}{{idx+1}}
                                </div>
                    ...
                    <div class="obottom">
                        合计：<span class="c-fc1 fs16"><sub>￥</sub>{{order.totalPrice}}</span>
                    </div>
                </div>
            </div>
        </div>
